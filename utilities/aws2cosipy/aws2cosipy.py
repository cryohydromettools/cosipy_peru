...
def create_2D_input(cs_file, cosipy_file, static_file, start_date, end_date, x0=None, x1=None, y0=None, y1=None):
    ...
    alb_interp_ice[alb_interp_ice > 0.40 ] = 0.40



    for i in range(len(ds.lat)):
        for j in range(len(ds.lon)):
            if ds.HGT.values[i,j] >= stationAlt:
               snow_interp[i,j] = snow_interp[i,j]
            else:
               snow_interp[i,j] = 0.0 
    ...
    add_variable_along_latlon(dso, alb_interp, 'ALBEDO_G', 'day', 'Albedo gradient')
    add_variable_along_latlon(dso, alb_interp_ice, 'ALBEDO_ICE', '', 'Albedo ice')
    add_variable_along_latlon(dso, snow_interp, 'SNOWHEIGHT', 'm', 'inicial snow layer')
