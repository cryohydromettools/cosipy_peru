...
G_nc   = ds['G']
G_day  = G_nc.resample(freq = 'd', dim = 'time', how = 'mean')
G_day  = G_day.values
...
A_nc   = ds['ALBEDO']
A_day  = A_nc.resample(freq = 'd', dim = 'time', how = 'mean')
A_day  = A_day.values

time_d,x,y = G_day.shape
...
LWin_nc    = ds['LWin']
LWin_day   = LWin_nc.resample(freq = 'd', dim = 'time', how = 'mean')
LWin_day   = LWin_day.values

LWout_nc   = ds['LWout']
LWout_day  = LWout_nc.resample(freq = 'd', dim = 'time', how = 'mean')
LWout_day  = LWout_day.values

time_d,x,y = LWout_day.shape

var2 = np.zeros((len(elev_g_20),time_d))

for t in range(time_d):
    var_nc_day  = LWin_day[t,:,:]
    var_nc_day1 = LWout_day[t,:,:]
    var_nc_z = np.zeros((len(elev_g),1))
    z = 0
    for i in range(lat_n):
        for j in range(lon_n):
...
H_nc   = ds['H']
H_day  = H_nc.resample(freq = 'd', dim = 'time', how = 'mean')
H_day  = H_day.values
...
LE_nc   = ds['LE']
LE_day  = LE_nc.resample(freq = 'd', dim = 'time', how = 'mean')
LE_day  = LE_day.values
...
QG_nc   = ds['B']
QG_day  = QG_nc.resample(freq = 'd', dim = 'time', how = 'mean')
QG_day  = QG_day.values
...
ME_nc   = ds['ME']
ME_day  = ME_nc.resample(freq = 'd', dim = 'time', how = 'mean')
ME_day  = ME_day.values
