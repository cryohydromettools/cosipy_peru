...
ME_all  = np.sum(ME_m.values,axis=0)

for i in range(len(lat_nc)):
    for j in range(len(lon_nc)):
            if mask_nc[i,j] == 1:
                ME_all[i,j] = ME_all[i,j]
            else:
                 ME_all[i,j] = np.nan


ME_all2  = ds['MB']
ME_day2  = ME_all2.resample(freq = 'd', dim = 'time', how = 'sum')
ME_m2    = ME_day2.resample(freq = 'm', dim = 'time', how = 'sum')
ME_all2  = np.sum(ME_m2.values,axis=0)
for i in range(len(lat_nc)):
    for j in range(len(lon_nc)):
            if mask_nc[i,j] == 1:
                ME_all2[i,j] = ME_all2[i,j]
            else:
                 ME_all2[i,j] = np.nan
...
im1 = plt.contourf(lon_nc,lat_nc,ME_all2,1)
p = im1.collections[1].get_paths()[0]
v = p.vertices
#y = savgol_filter(x, 5, 2)

fig, ax = plt.subplots(figsize=(4,5))
im  = ax.contourf(lon_nc,lat_nc,ME_all,20,vmin=-13, vmax=1, cmap=cmap_r)
